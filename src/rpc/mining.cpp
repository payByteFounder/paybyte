...
static bool ScriptPubKeyFromAddress(const std::string& addr, CScript& out)
{
...
static RPCHelpMan generatetodescriptor()
{
    return RPCHelpMan{
        ...
        "\nMine blocks immediately to a specified descriptor (before the RPC call returns)\n",
        {
            ...
            {"descriptor", RPCArg::Type::STR, RPCArg::Optional::NO, "The descriptor to send the newly generated Paybyte to."},
...
static RPCHelpMan generatetoaddress()
{
    return RPCHelpMan{"generatetoaddress",
                "\nMine blocks immediately to a specified address (before the RPC call returns)\n",
                {
                    ...
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address to send the newly generated Paybyte to."},
                    {"maxtries", RPCArg::Type::NUM, /* default */ ToString(DEFAULT_MAX_TRIES), "How many iterations to try."},
                },
                RPCResult{
                    RPCResult::Type::ARR, "", "hashes of blocks generated",
            ...
            + "If you are using the " PACKAGE_NAME " wallet, you can get a new address to send the newly generated Paybyte to with:\n"
...
static RPCHelpMan generateblock()
{
    return RPCHelpMan{"generateblock",
        ...
        {
            {"output", RPCArg::Type::STR, RPCArg::Optional::NO, "The address or descriptor to send the newly generated Paybyte to."},
...
static RPCHelpMan startcpumining()
{
    return RPCHelpMan{
        "startcpumining",
        "Start the built-in CPU miner.",
        {
            {"threads", RPCArg::Type::STR, RPCArg::Optional::NO, "Number of CPU threads"},
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "PayByte address to mine to"},
        },
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                {RPCResult::Type::BOOL, "started", "Whether mining started"},
                {RPCResult::Type::NUM, "threads", "Threads in use"},
            }
        },
        RPCExamples{
            HelpExampleCli("startcpumining", "2 pbe1q...")
        },
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            NodeContext& node = EnsureNodeContext(request.context);

            const int threads = std::stoi(request.params[0].get_str());
            const std::string addr = request.params[1].get_str();

            CScript scriptPubKey;
            if (!ScriptPubKeyFromAddress(addr, scriptPubKey)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
            }

            const bool ok = node.miner->Start(threads, scriptPubKey);

            UniValue ret(UniValue::VOBJ);
            ret.pushKV("started", ok);
            ret.pushKV("threads", node.miner->GetThreads());
            return ret;
        }
    };
}
...
static RPCHelpMan stopcpumining()
{
    return RPCHelpMan{
        "stopcpumining",
        "Stop the built-in CPU miner.",
        {},
        RPCResult{RPCResult::Type::OBJ, "", "",
        {
            {RPCResult::Type::BOOL, "stopped", "Whether mining stopped"},
        }},
        RPCExamples{
        ...
        {
            NodeContext& node = EnsureNodeContext(request.context);
            if (!node.miner) throw JSONRPCError(RPC_INTERNAL_ERROR, "CPU miner not initialized");

            const bool was_running = node.miner->IsRunning();
            node.miner->Stop();

            UniValue ret(UniValue::VOBJ);
            ret.pushKV("stopped", was_running);
            return ret;
        }
    };
}
...
static RPCHelpMan getcpumininginfo()
{
    return RPCHelpMan{
        "getcpumininginfo",
        "Get status of the built-in CPU miner.",
        {},
        RPCResult{RPCResult::Type::OBJ, "", "",
        {
            {RPCResult::Type::BOOL, "running", "Is CPU miner running"},
            {RPCResult::Type::NUM, "threads", "Threads in use"},
            {RPCResult::Type::NUM, "hashes", "Total hashes attempted"},
            {RPCResult::Type::NUM, "blocks_found", "Blocks found by CPU miner"},
        }},
        RPCExamples{
        ...
        [](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            UniValue ret(UniValue::VOBJ);
            ret.pushKV("running", node.miner->IsRunning());
            ret.pushKV("threads", node.miner->GetThreads());
            ret.pushKV("hashes", (uint64_t)node.miner->GetHashes());
            ret.pushKV("blocks_found", (uint64_t)node.miner->GetBlocksFound());
            return ret;
        }
    };
}
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    // ---- PayByte Built-In CPU Miner ----
    { "mining",             "startcpumining",         &startcpumining,         {"threads","address"} },
    { "mining",             "stopcpumining",          &stopcpumining,          {} },
    { "mining",             "getcpumininginfo",       &getcpumininginfo,       {} },
