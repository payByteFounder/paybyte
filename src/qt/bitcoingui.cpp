...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Paybyte address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and paybyte: URIs)"));
    ...
    mineAction = new QAction(
        platformStyle->SingleColorIcon(":/icons/tx_mined"),
        tr("&Mine"),
        this
    );
    mineAction->setStatusTip(tr("One click mining to the active wallet"));
    ...
    connect(mineAction, &QAction::toggled, this, &BitcoinGUI::toggleMining);
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Paybyte addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Paybyte addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file..."), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Paybyte Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from clipboard..."), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Paybyte Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a paybyte: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Paybyte command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to the PayByte network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void BitcoinGUI::toggleMining(bool checked)
{
    LogPrintf("GUI TOGGLE MINING CALLED: %d\n", checked);
    ...
    WalletModel* walletModel = view->getWalletModel();
    ...
    {
        CTxDestination dest;
        if (!walletModel->wallet().getNewDestination(OutputType::LEGACY, "", dest)) {
            mineAction->setChecked(false);
            return;
        }
        ...
        // Hard cap at 4 threads to prevent internal fork racing
        if (threads > 4)
            threads = 4;

        if (!node.startCPUMining(threads, scriptPubKey)) {
            mineAction->setChecked(false);
            return;
        }

        QString walletName = walletModel->getDisplayName();
        mineAction->setText(QString("Mining → %1").arg(walletName));
    }
    else
    {
        node.stopCPUMining();
        mineAction->setText(tr("Mine"));
    }
}