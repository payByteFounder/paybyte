...
public:
    ...
    bool startCPUMining(int threads, const CScript& scriptPubKey) override
    {
        if (!m_node.miner) return false;
        return m_node.miner->Start(threads, scriptPubKey);
    }
    ...
    bool isCPUMining() const override
    {
        return m_node.miner && m_node.miner->IsRunning();
    }
